...
[Authorize]
public sealed class MenuOpcionesController(IMenuOpcionesService menuService) : ControllerBase
{
    ...
    public async Task<ActionResult<IReadOnlyList<MenuOpcionDto>>> GetAll(CancellationToken ct)
    {
        ...
        return Ok(await menuService.GetAllAsync(ct));
    }
    ...
    {
        if (!EsAdmin(User)) return Forbid();
        var m = await menuService.GetByIdAsync(id, ct);
    ...
    {
        if (!EsAdmin(User)) return Forbid();
        var (created, err) = await menuService.CreateAsync(dto, ct);
    ...
    {
        if (!EsAdmin(User)) return Forbid();
        var err = await menuService.UpdateAsync(id, dto, ct);
    ...
    {
        if (!EsAdmin(User)) return Forbid();
        var err = await menuService.DeleteAsync(id, ct);
        if (err == "NOT_FOUND") return NotFound();
