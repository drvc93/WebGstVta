...
{
    private readonly IMaestroService<Unidad> _crud;

    public UnidadesController(IMaestroService<Unidad> crud)
    {
        _crud = crud;
    }

    [HttpGet]
    public async Task<ActionResult<List<Unidad>>> GetAll(CancellationToken ct) => Ok(await _crud.GetAllAsync(ct));

    [HttpGet("{id:int}")]
    public async Task<ActionResult<Unidad>> GetById(int id, CancellationToken ct)
    {
    ...
    [HttpPost]
    public async Task<ActionResult<Unidad>> Create([FromBody] Unidad entity, CancellationToken ct)
    {
    ...
    [HttpPut("{id:int}")]
    public async Task<IActionResult> Update(int id, [FromBody] Unidad dto, CancellationToken ct)
    {
