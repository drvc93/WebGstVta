...
{
    private readonly IMaestroService<Pais> _crud;

    public PaisesController(IMaestroService<Pais> crud)
    {
        _crud = crud;
    }

    [HttpGet]
    public async Task<ActionResult<List<Pais>>> GetAll(CancellationToken ct) => Ok(await _crud.GetAllAsync(ct));

    [HttpGet("{id:int}")]
    public async Task<ActionResult<Pais>> GetById(int id, CancellationToken ct)
    {
    ...
    [HttpPost]
    public async Task<ActionResult<Pais>> Create([FromBody] Pais entity, CancellationToken ct)
    {
    ...
    [HttpPut("{id:int}")]
    public async Task<IActionResult> Update(int id, [FromBody] Pais dto, CancellationToken ct)
    {
