...
{
    private readonly IMaestroService<Marca> _crud;

    public MarcasController(IMaestroService<Marca> crud)
    {
        _crud = crud;
    }

    [HttpGet]
    public async Task<ActionResult<List<Marca>>> GetAll(CancellationToken ct) => Ok(await _crud.GetAllAsync(ct));
    ...
    [HttpPut("{id:int}")]
    public async Task<IActionResult> Update(int id, [FromBody] Marca dto, CancellationToken ct)
    {
