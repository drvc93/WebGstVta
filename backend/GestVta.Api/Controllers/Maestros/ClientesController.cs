...
{
    private readonly IMaestroService<Cliente> _crud;

    public ClientesController(IMaestroService<Cliente> crud)
    {
        _crud = crud;
    }

    [HttpGet]
    public async Task<ActionResult<List<Cliente>>> GetAll(CancellationToken ct) => Ok(await _crud.GetAllAsync(ct));
    ...
    [HttpPost]
    public async Task<ActionResult<Cliente>> Create([FromBody] Cliente entity, CancellationToken ct)
    {
    ...
    [HttpPut("{id:int}")]
    public async Task<IActionResult> Update(int id, [FromBody] Cliente dto, CancellationToken ct)
    {
