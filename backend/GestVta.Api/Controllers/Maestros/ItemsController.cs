...
{
    private readonly IMaestroService<Item> _crud;

    public ItemsController(IMaestroService<Item> crud)
    {
        _crud = crud;
    }

    [HttpGet]
    public async Task<ActionResult<List<Item>>> GetAll(CancellationToken ct) => Ok(await _crud.GetAllAsync(ct));
    ...
    [HttpPost]
    public async Task<ActionResult<Item>> Create([FromBody] Item entity, CancellationToken ct)
    {
    ...
    [HttpPut("{id:int}")]
    public async Task<IActionResult> Update(int id, [FromBody] Item dto, CancellationToken ct)
    {
