...
{
    private readonly IMaestroService<Proveedor> _crud;

    public ProveedoresController(IMaestroService<Proveedor> crud)
    {
        _crud = crud;
    }

    [HttpGet]
    public async Task<ActionResult<List<Proveedor>>> GetAll(CancellationToken ct) => Ok(await _crud.GetAllAsync(ct));

    [HttpGet("{id:int}")]
    public async Task<ActionResult<Proveedor>> GetById(int id, CancellationToken ct)
    {
    ...
    [HttpPost]
    public async Task<ActionResult<Proveedor>> Create([FromBody] Proveedor entity, CancellationToken ct)
    {
    ...
    [HttpPut("{id:int}")]
    public async Task<IActionResult> Update(int id, [FromBody] Proveedor dto, CancellationToken ct)
    {
