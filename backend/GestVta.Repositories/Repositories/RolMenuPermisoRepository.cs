...
public sealed class RolMenuPermisoRepository(ApplicationDbContext db) : IRolMenuPermisoRepository
{
    public async Task<IReadOnlyList<RolMenuPermiso>> ListByRolIdNoTrackingAsync(int rolId, CancellationToken ct) =>
        await db.RolMenuPermisos.AsNoTracking().Where(p => p.RolId == rolId).ToListAsync(ct);
    ...
    public async Task<IReadOnlyList<RolMenuPermiso>> ListByRolIdsNoTrackingAsync(IReadOnlyList<int> rolIds, CancellationToken ct)
    {
        ...
        return await db.RolMenuPermisos.AsNoTracking().Where(p => rolIds.Contains(p.RolId)).ToListAsync(ct);
    }

    public async Task<IReadOnlyList<RolMenuPermiso>> ListTrackedByRolIdAsync(int rolId, CancellationToken ct) =>
        await db.RolMenuPermisos.Where(p => p.RolId == rolId).ToListAsync(ct);

    public async Task<IReadOnlyList<RolMenuPermiso>> ListTrackedByMenuOpcionIdAsync(int menuOpcionId, CancellationToken ct) =>
        await db.RolMenuPermisos.Where(p => p.MenuOpcionId == menuOpcionId).ToListAsync(ct);

    public void RemoveRange(IEnumerable<RolMenuPermiso> entities) => db.RolMenuPermisos.RemoveRange(entities);

    public void Add(RolMenuPermiso entity) => db.RolMenuPermisos.Add(entity);
