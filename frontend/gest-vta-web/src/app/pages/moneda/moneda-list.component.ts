...
type SortKey = keyof Pick<Moneda, 'codigo' | 'simbolo' | 'nombre' | 'activo'>;

@Component({
  selector: 'app-moneda-list',
  templateUrl: './moneda-list.component.html',
  styleUrl: './moneda-list.component.scss',
})
export class MonedaListComponent implements OnInit {
  ...
  readonly filteredRows = computed(() => {
    ...
    return this.rows().filter((m) =>
      [m.codigo, m.simbolo, m.nombre, m.activo ? 'activo' : 'inactivo'].some((x) =>
  ...
  ngOnInit(): void {
    this.api.getAll().subscribe({
      ...
      error: () => {
        this.error.set('No se pudo cargar la lista de monedas.');
        this.loading.set(false);
      },
    });
  }
  ...
  private sortValue(m: Moneda, key: SortKey): string | number {
    switch (key) {
      case 'codigo':
      case 'simbolo':
      case 'nombre':
