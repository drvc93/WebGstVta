...
export type UsuarioListSortKey = 'username' | 'nombreMostrar' | 'activo' | 'fechaCreacion';
...
export class UsuarioListStore {
  ...
  readonly sortKey = signal<UsuarioListSortKey>('username');
  ...
  load(): void {
    this.companiasApi.getAll().subscribe({
      ...
      error: () => {
        /* la lista puede mostrar ids si fallan nombres */
      },
    });
  ...
  companiasLabel(u: UsuarioListItem): string {
    ...
    return (u.companiaIds ?? []).map((id) => map.get(id) ?? `#${id}`).join(', ');
  }
  ...
  toggleSort(key: UsuarioListSortKey): void {
    if (this.sortKey() === key) {
  ...
  sortIconState(key: UsuarioListSortKey): 'asc' | 'desc' | 'none' {
  ...
  private sortValue(u: UsuarioListItem, key: UsuarioListSortKey): string | number {
    switch (key) {
      case 'username':
      case 'nombreMostrar':
