...
type SortKey = keyof Pick<TipoDocumento, 'codigo' | 'nombre' | 'activo'>;
...
export class TipoDocumentoListComponent implements OnInit {
  ...
  readonly rows = signal<TipoDocumento[]>([]);
  ...
  readonly filteredRows = computed(() => {
    ...
    return this.rows().filter((m) =>
      [m.codigo, m.nombre, m.activo ? 'activo' : 'inactivo'].some((x) => (x ?? '').toString().toLowerCase().includes(q)),
    );
  });
  ...
  private sortValue(m: TipoDocumento, key: SortKey): string | number {
    switch (key) {
      case 'codigo':
      case 'nombre':
